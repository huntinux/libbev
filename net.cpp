...
conn *worker_thread_conn_new(const int sfd, enum conn_states init_state,
                const int event_flags,
                const int read_buffer_size, enum network_transport transport,
                struct event_base *base) ;

conn *main_thread_conn_new(const int sfd, enum conn_states init_state,
                const int event_flags,
                const int read_buffer_size, enum network_transport transport,
                struct event_base *base);
/*
...
static void thread_libevent_process(int fd, short which, void *arg) {
    ...
    case 'c':
        ...
        switch (item.mode) {
            case queue_new_conn:
                c = worker_thread_conn_new(item.sfd, item.init_state, item.event_flags, 1024, tcp_transport, me->base);
                if (c == NULL) {
...
static void main_thread_drive_machine(conn *c)
{
    bool stop = false;
    int sfd;
    socklen_t addrlen;
    ...
    while (!stop) {
        ...
        case conn_listening:
            ...
            dispatch_conn_new(sfd, conn_established, EV_READ | EV_PERSIST, 1024);
...
void main_thread_event_handler(const int fd, const short which, void *arg)
{
...
void worker_thread_readcb(struct bufferevent *bev, void *ctx)
{
	char buffer[1024];
	size_t nr = bufferevent_read(bev, buffer, sizeof buffer);
	int r = bufferevent_write(bev, buffer, nr);
	if(r != 0) {
		fprintf(stderr, "bufferevent write failed\n");
		conn* c = (conn*)ctx;
		bufferevent_free(c->be);
		free(c);
	}
}

readcb g_rcb = worker_thread_readcb;
...
void worker_thread_eventcb(struct bufferevent *bev, short error, void *ctx)
{
    ...
    fprintf(stdout, "Peer close the connection\n");
    conn* c = (conn*)ctx;
    bufferevent_free(c->be);
    free(c);
}
...
conn *worker_thread_conn_new(const int sfd, enum conn_states init_state,
    ...
    c->sfd   = sfd;
    c->state = init_state;
    c->be    = bufferevent_socket_new(base, sfd, BEV_OPT_CLOSE_ON_FREE);
    c->ev_flags = event_flags;

    bufferevent_setcb(c->be, g_rcb, NULL, worker_thread_eventcb, (void*)c);
    bufferevent_setwatermark(c->be, EV_READ, 0, 1024 * 64);
...
conn *main_thread_conn_new(const int sfd, enum conn_states init_state,
    ...
    event_assign(&c->event, base, sfd, event_flags, main_thread_event_handler, (void *)c);
    c->ev_flags = event_flags;
...
int server_socket(int port)
{
    ...
    for (next = ai; next; next = next->ai_next) {
        ...
        if (!(main_thread_conn_new(sfd, conn_listening, EV_READ | EV_PERSIST, 1, transport, main_base))) {
...
void start_work(int worker_thread_num, short port, readcb rcb)
{
	...
	thread_init(worker_thread_num);
	server_socket(port);
	set_worker_thread_readcb(rcb);
